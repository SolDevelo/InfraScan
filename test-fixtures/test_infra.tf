...
# ── Additional CRITICAL Checkov violations ────────────────────────────────────

# CKV_AWS_126 / CKV_AWS_131: ECS task definition with root user and host networking
resource "aws_ecs_task_definition" "app" {
  family                   = "app-task"
  network_mode             = "host" # CKV_AWS_131: host networking exposes host ports
  requires_compatibilities = ["EC2"]

  # CKV_AWS_97: no EFS encryption in transit
  # CKV_AWS_126: container runs as root (no user set, no readonlyRootFilesystem)
  container_definitions = jsonencode([{
    name      = "app"
    image     = "nginx:1.14"           # old, vulnerable image tag
    essential = true
    privileged = true                  # CKV_AWS_126: privileged container
    user      = "root"                 # running as root inside ECS task
    ...
    environment = [{
      name  = "DB_PASSWORD"
      value = "SuperSecret123!"        # CKV_SECRET_6: hardcoded secret in task def
    }]
  }])
}

# CKV_AWS_58: EKS secrets not encrypted with KMS
resource "aws_eks_cluster" "main" {
  ...
  vpc_config {
    ...
    endpoint_public_access  = true  # CKV_AWS_58: public API server endpoint
    endpoint_private_access = false
    public_access_cidrs     = ["0.0.0.0/0"] # CKV_AWS_58: open to all IPs
  }

  # CKV_AWS_58: no envelope encryption for secrets
  # (encryption_config block absent)

  # CKV_AWS_37: no audit logging
  # (enabled_cluster_log_types absent)
}

# CKV_AWS_57: Elasticsearch/OpenSearch domain with public access
resource "aws_elasticsearch_domain" "search" {
  domain_name           = "prod-search"
  elasticsearch_version = "6.3" # old version, known CVEs

  # CKV_AWS_84: no Cognito/advanced security
  # CKV_AWS_83: no node-to-node encryption
  # CKV_AWS_5:  no encryption at rest
  encrypt_at_rest {
    enabled = false
  }
  ...
  # CKV_AWS_57: open access policy (public)
...
# CKV_AWS_18: S3 bucket with public access and no access logging
resource "aws_s3_bucket" "public_assets" {
  bucket = "my-company-public-assets"
  acl    = "public-read-write" # CKV_AWS_20: public read-write ACL
}

# CKV_AWS_54 / CKV_AWS_57: SNS topic without server-side encryption
resource "aws_sns_topic" "alerts" {
  name = "prod-alerts"
  # kms_master_key_id absent → no encryption
}
