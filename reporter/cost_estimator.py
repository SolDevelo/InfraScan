...
def _findings_section_md(
    ...
    *limit* controls how many rows appear; None = all.  0 = omit entirely.
    ...
    for f in shown:
        ...
        desc = f.get("description", f.get("name", ""))[:80]
        rows.append(f"| {icon} {sev} | {rid} | {loc} | {desc} |")
    ...
    return [
        f"<details>",
        f"<summary><b>{title} ({count})</b></summary>",
        "",
    ] + table + [
        "",
        "</details>",
...
def format_ci_summary_md(
    ...
    *baseline* is a previously saved report_dict used to compute deltas.
    ...
    def _grade_row(name: str, g: dict) -> None:
        ...
        grade_rows.append(f"| {name} | {icon} **{letter}** ({pct}%) | {detail} |")
    ...
    # ── 2 & 3. Security + container findings by severity ─────────────────────
    ...
    # HIGH — collapsed
    high_sec  = _by_sev(all_sec, "high")
    high_cont = _by_sev(all_cont, "high")
    high_cost = _by_sev(all_cost, "high")
    high_all  = high_sec[:sec_lim.get("high") or len(high_sec)] + \
                high_cont[:cont_lim.get("high") or len(high_cont)] + \
                high_cost[:cost_lim.get("high") or len(high_cost)]
    if high_all:
        total_high = len(_by_sev(all_sec, "high") + _by_sev(all_cont, "high") + _by_sev(all_cost, "high"))
        lines += _findings_section_md(f"🟠 HIGH findings", high_all,
                                       limit=len(high_all),
                                       always_expanded=False)

    # MEDIUM — collapsed; containers omitted when limit=0
    med_sec  = _by_sev(all_sec, "medium")[:sec_lim.get("medium") or 0]
    med_cont_limit = cont_lim.get("medium", 0)
    med_cont = _by_sev(all_cont, "medium")[:(med_cont_limit or 0)]
    med_cost = _by_sev(all_cost, "medium")[:cost_lim.get("medium") or 0]

    all_med_src = _by_sev(all_sec, "medium") + _by_sev(all_cont, "medium") + _by_sev(all_cost, "medium")
    shown_med = med_sec + med_cont + med_cost
    if shown_med:
        lines += _findings_section_md(f"🟡 MEDIUM findings",
                                       shown_med,
                                       limit=len(shown_med),
                                       always_expanded=False)
        overflow = len(all_med_src) - len(shown_med)
        if overflow > 0 and med_cont_limit == 0:
            lines += [f"> ℹ️ {len(_by_sev(all_cont, 'medium'))} container MEDIUM findings omitted "
                      f"(base-image CVEs). See full HTML report.", ""]
    ...
    if total_cost:
        ...
        if baseline:
            ...
            if base_cost:
                ...
                if delta > 0:
                    delta_str = f" (**+{_fmt_usd(delta)}/mo ⚠️** vs baseline)"
                elif delta < 0:
                    delta_str = f" (**{_fmt_usd(delta)}/mo ✅** vs baseline)"
        lines.append(f"💵 **Infrastructure cost:** {_fmt_usd(total_cost)}/mo{delta_str}")
        lines.append("")
    ...
    if per_finding:
        ...
        rows_md = ["| Rule | File | Saving/month |", "|---|---|---|"]
        for pf in per_finding[:10]:
            ...
            rows_md.append(f"| {pf.get('rule_id', '')} | {fname}:{line_n} | {s_str} |")
        lines += [
            "<details>",
    ...
    if run_url:
        lines.append(f"→ [Full HTML report]({run_url})")
...
def format_pr_comment_md(
    ...
    """
    Return a compact PR comment.  Only posted when actionable:
    - new CRITICAL findings (always), or
    - new HIGH findings when alert_on='critical_high', or
    - cost delta > $5/mo or > 10%.

    Returns empty string when there is nothing worth posting.
    """
    findings = report_dict.get("findings", {})
    metrics  = report_dict.get("metrics", {})
    savings  = metrics.get("savings_estimate", {})
    ...
    should_post = bool(new_crits)
    if alert_on == "critical_high":
        should_post = should_post or bool(new_highs)
    should_post = should_post or (cost_delta > 5 or cost_delta_pct > 10)
    ...
    if base_cost and cost_delta != 0:
        ...
        lines += [f"💵 Infra cost: **{_fmt_usd(total_cost)}/mo**", ""]
    ...
    if run_url:
        lines.append(f"→ Full report: [{run_url}]({run_url})")
